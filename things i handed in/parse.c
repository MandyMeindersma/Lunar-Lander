...
void make_star_array(){
	//getting a 'random' star location
...
void check_hit_token(){
  int m;
  int intersseeecccted;
  for (m=0;m<=19;m=m+2){
  	intersseeecccted=lineSegmentIntersection(token_array[m][0],token_array[m][1],token_array[m+1][0],token_array[m+1][1], 
                                             ship_array[1][0],ship_array[1][1],ship_array[2][0],ship_array[2][1]);
    if (intersseeecccted==1){
      drawstar=0;
...
void fuel_bar(){
		//if statements for color change*/
		if (fuel_level<50){
		fprintf(sketch, "setColor 238 0 0\n");
		}
		else if (fuel_level<90){
		fprintf(sketch, "setColor 255 215 0\n");
		}
		else{
		fprintf(sketch, "setColor 0 139 0\n");
		}
		//print fuel bar
		fprintf(sketch, "drawSegment 10 10 %li 10\n", lround(fuel_level));
		fprintf(sketch, "drawSegment 10 11 %li 11\n", lround(fuel_level));
		fprintf(sketch, "drawSegment 10 12 %li 12\n", lround(fuel_level));
		fprintf(sketch, "drawSegment 10 13 %li 13\n", lround(fuel_level));
		fprintf(sketch, "setColor 0 0 0\n");
		fflush(sketch);
}/*end fuel_bar*/