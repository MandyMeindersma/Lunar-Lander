  ...
  if( signal == SIGALRM ) {
    ...
    if (improvements==1){
      ...
      for (m=0;m<number_of_landscape_points-1;m++){
        ...
        if ((they_intersect==1) && ((a==90)||(a==-270)) && (v<max_y_velocity) && (array_of_landscape[m][1]==array_of_landscape[m+1][1]) ){
    ...
    if (done==0 && improvements==1){fuel_bar();}
  }/*end signal thing*/
...
//getting random lines and printing them
void draw_crashed(){
  double the_middle_x;
  double the_middle_y;
