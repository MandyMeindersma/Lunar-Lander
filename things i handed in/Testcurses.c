...
//I TOOK SOME OF THIS CODE FROM ECLASS
int test_curses(){
  yA=gravity;
  xA=0;
  ...
  do {
    ...
    case ' ':
    if (done==0 && improvements==1){
      if (fuel_level<10){fuel_level=10;}
      if (fuel_level>10){
        spacebar=1;
        if ((what_time_is_it%1)==0){fuel_level=fuel_level-space_bar_dec;}
        if (fuel_level<10){fuel_level=10;}
        xA = thrust * cos(angle_of_ship);
